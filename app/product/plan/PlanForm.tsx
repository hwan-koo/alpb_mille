...
"
        ...
        <div>
          <div className="w-full bg-[#F7F7F7] h-56 mt-2 p-4 flex flex-col justify-center items-center">
            ...
            <button
              className="w-24 flex flex-row justify-center items-center text-white bg-[#333333] text-sm p-2 rounded-md "
              type="button"
              onClick={() => {
                setShow(!show);
              }}
            >
              ...
              <span className="ml-1">표지 선택</span>
            </button>
          </div>
        </div>
        <button
          className={`w-full  rounded-md p-2 mt-4 ${
            title.length < 2 || introduction.length < 2
              ? "bg-gray-400 text-gray-200"
              : "bg-[#FAE65E]"
          }`}
          disabled={title.length < 2 || introduction.length < 2}
        >
          저장하기
        </button>
        <input
        ...
        <input
          className="hidden"
          defaultValue={(imageTimeStamp! as string) ?? ""}
          name="imageTimeStamp"
        ></input>
      </form>
      <CoverChoiceModal
        ...
        user_id={user_id}
        deliverImageTimeStamp={getTimestamp}
        title={title}
